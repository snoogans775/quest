...
function Post()
{
	...
	if (isset($_REQUEST['message']) || !empty($context['post_error']))
	{
		...
		if ($really_previewing == true || isset($_REQUEST['xml']))
		{
			...
			$msg_id = 'pre' . (isset($_REQUEST['msg']) ? $_REQUEST['msg'] : '');
			...
			$context['preview_message'] = ILA_Invalid_Tags($context['preview_message'], $msg_id);
	...
	else
	{
		...
		if (!empty($topic) && !empty($_REQUEST['quote']))
		{
			...
			$form_message = ILA_Add_MsgID($form_message);
		}
...
function Post2()
{
	...
	// MOVED FURTHER DOWN: Check if they are trying to delete any current attachments....
	...
	if (isset($_FILES['attachment']['name']) || (!empty($_SESSION['temp_attachments']) && empty($_POST['from_qr'])))
	{
		...
		foreach ($_FILES['attachment']['tmp_name'] as $n => $dummy)
		{
			...
			{
				$_POST['attach_del'][] = $attachIDs[] = $attachmentOptions['id'];
...
function getTopic()
{
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		// Show attachment string or error message in topic history
		$row['body'] = ILA_Invalid_Tags($row['body'], $row['id_msg']);
