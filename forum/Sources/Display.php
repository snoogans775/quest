...
function Download()
{
	...
	elseif (ILA_Mime_Type($file_ext, $mime_type))
...
function loadAttachmentContext($id_msg, $override_show = false)
{
	...
	if (isset($attachments[$id_msg]) && !empty($modSettings['attachmentEnable']))
	{
		foreach ($attachments[$id_msg] as $i => $attachment)
		{
			$attachmentData[$i] = array(
				...
				'is_image' => !empty($attachment['width']) && !empty($attachment['height']) && (!empty($modSettings['attachmentShowImages']) || $override_show),
